...
public class BlueThermalPrinterPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, RequestPermissionsResultListener {
  ...
  // ── FIXED connect() — insecure fallback + stale thread cleanup ──────────────
  private void connect(Result result, String address) {

    // Force-clear any stale thread instead of refusing to connect
    if (THREAD != null) {
      try { THREAD.cancel(); } catch (Exception ignored) {}
      THREAD = null;
    }
    ...
    AsyncTask.execute(() -> {
      try {
        ...
        // Always cancel discovery — active discovery kills socket connections
        mBluetoothAdapter.cancelDiscovery();

        BluetoothSocket socket = null;

        // Try secure RFCOMM first, fall back to insecure for Android 12+ compatibility
        try {
          socket = device.createRfcommSocketToServiceRecord(MY_UUID);
          socket.connect();
          Log.i(TAG, "Connected via secure RFCOMM");
        } catch (Exception e1) {
          Log.w(TAG, "Secure connect failed (" + e1.getMessage() + "), trying insecure...");
          try {
            if (socket != null) {
            ...
            Log.e(TAG, "Insecure connect also failed: " + e2.getMessage());
            if (socket != null) {
              try { socket.close(); } catch (Exception ignored) {}
            }
  ...
  private void printCustom(Result result, String message, int size, int align, String charset) {
    byte[] cc  = new byte[]{0x1B, 0x21, 0x03}; // 0 - normal
    byte[] bb  = new byte[]{0x1B, 0x21, 0x08}; // 1 - bold
    byte[] bb2 = new byte[]{0x1B, 0x21, 0x20}; // 2 - bold medium
    byte[] bb3 = new byte[]{0x1B, 0x21, 0x10}; // 3 - bold large
    byte[] bb4 = new byte[]{0x1B, 0x21, 0x30}; // 4 - strong
    byte[] bb5 = new byte[]{0x1B, 0x21, 0x50}; // 5 - extra strong
    ...
    try {
      switch (size) {
        case 0: THREAD.write(cc);  break;
        case 1: THREAD.write(bb);  break;
        case 2: THREAD.write(bb2); break;
        case 3: THREAD.write(bb3); break;
        case 4: THREAD.write(bb4); break;
        case 5: THREAD.write(bb5); break;
      }
      switch (align) {
        case 0: THREAD.write(PrinterCommands.ESC_ALIGN_LEFT);   break;
        case 1: THREAD.write(PrinterCommands.ESC_ALIGN_CENTER); break;
        case 2: THREAD.write(PrinterCommands.ESC_ALIGN_RIGHT);  break;
      }
      if (charset != null) {
  ...
  private void printLeftRight(Result result, String msg1, String msg2, int size, String charset, String format) {
    ...
    byte[] bb4 = new byte[]{0x1B, 0x21, 0x30};
    ...
    try {
      ...
      if (format != null) line = String.format(format, msg1, msg2);
      if (charset != null) {
  ...
  private void print3Column(Result result, String msg1, String msg2, String msg3, int size, String charset, String format) {
    ...
    try {
      ...
      if (format != null) line = String.format(format, msg1, msg2, msg3);
      if (charset != null) {
        THREAD.write(line.getBytes(charset));
      } else {
        THREAD.write(line.getBytes());
  ...
  private void print4Column(Result result, String msg1, String msg2, String msg3, String msg4, int size, String charset, String format) {
    ...
    try {
      ...
      if (format != null) line = String.format(format, msg1, msg2, msg3, msg4);
      if (charset != null) {
        THREAD.write(line.getBytes(charset));
      } else {
        THREAD.write(line.getBytes());
  ...
  private void printImage(Result result, String pathImage) {
    ...
    try {
      ...
      if (bmp != null) {
        ...
        Log.e("Print Photo error", "file not found: " + pathImage);
      }
      result.success(true);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
  ...
  private void printImageBytes(Result result, byte[] bytes) {
    ...
    try {
      ...
      if (bmp != null) {
        ...
        Log.e("Print Photo error", "failed to decode image bytes");
      }
      result.success(true);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
  ...
  private void printQRcode(Result result, String textToQR, int width, int height, int align) {
    ...
    try {
      ...
      if (bmp != null) {
        ...
        Log.e("Print QR error", "failed to generate QR bitmap");
      }
      result.success(true);
    } catch (Exception ex) {
      Log.e(TAG, ex.getMessage(), ex);
