...
class _VyaparHomeScreenState extends State<VyaparHomeScreen>
  ...
  Future<void> _sharePdf(int invoiceId) async {
    ...
    if (!mounted) return;
    await Printing.sharePdf(bytes: bytes, filename: '${inv['invoice_no'] ?? 'bill'}.pdf');
  }

  Future<void> _shareImage(int invoiceId) async {
    final settings = context.read<SettingsProvider>();
    final inv = await _db.getInvoiceById(invoiceId);
    if (!mounted || inv == null) return;
    final items = (inv['items'] as List?)
    ...
    try {
      ...
      await for (final page in Printing.raster(bytes, pages: [0], dpi: 300)) {
        ...
        if (mounted) {
          await Printing.sharePdf(bytes: imgBytes,
              filename: '${inv['invoice_no'] ?? 'bill'}.png');
        }
        break;
      }
    } catch (e) {
  ...
  Future<void> _printThermal(int invoiceId) async {
    ...
    final err = await ThermalInvoicePrinter.printInvoice(
        settings: settings, invoice: inv, items: items);
    if (!mounted) return;
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Row(children: [
                Container(width: 38, height: 38,
                  ...
                  child: const Icon(Icons.person_add_alt_1, color: _kBlue, size: 18)),
              ...
              _Field(ctrl: nameCtrl, label: 'Name *', icon: Icons.person_outline,
                  autofocus: true, caps: TextCapitalization.words),
              const SizedBox(height: 10),
              _Field(ctrl: phoneCtrl, label: 'Phone (optional)',
                  icon: Icons.phone_outlined, keyboard: TextInputType.phone),
              const SizedBox(height: 10),
              _Field(ctrl: addrCtrl, label: 'Address (optional)',
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: partyType == CustomerModel.typeCustomer
                        ? _kBlue : Colors.indigo,
  ...
  Future<void> _deleteParty(CustomerModel c) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('âŒ Delete failed: $e'), backgroundColor: Colors.red));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        _VyaparHeader(
          shopName: settings.shopName.isEmpty ? 'Godawari Fish' : settings.shopName,
          today: today, toReceive: toReceive, toPay: toPay, sym: sym,
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => Padding(
                  ...
                  child: _VyaparTxnRow(row: merged[i], sym: sym,
                    onTap: () {
                    ...
                    onPrint:  merged[i]['_kind'] == 'sale' && merged[i]['id'] != null
                        ? () => _printThermal(merged[i]['id'] as int) : null,
                    onShare:  merged[i]['_kind'] == 'sale' && merged[i]['id'] != null
                        ? () => _sharePdf(merged[i]['id'] as int) : null,
                    onImage:  merged[i]['_kind'] == 'sale' && merged[i]['id'] != null
                        ? () => _shareImage(merged[i]['id'] as int) : null,
                  ),
                ),
    ...
    return Padding(
      ...
      child: Column(children: [
        Container(width: 68, height: 68,
          ...
          child: const Icon(Icons.receipt_long_outlined, size: 32, color: _kRed)),
        const SizedBox(height: 14),
  ...
  Widget _buildParties(String sym) {
    return Consumer<CustomerProvider>(
      builder: (context, cp, _) {
            ...
            children: [
              ...
              Center(child: Column(children: [
                Container(width: 68, height: 68,
                  ...
                  child: const Icon(Icons.groups_2_outlined, size: 32, color: _kRed)),
                const SizedBox(height: 14),
        ...
        final toGet  = list.where((c) => c.balance > 0.009)
            .fold<double>(0, (s, c) => s + c.balance);
        final toGive = list.where((c) => c.balance < -0.009)
          ...
          ListView(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(children: [
                  Expanded(child: _SummaryPill(
                    label: "You'll Get", value: '$sym${toGet.toStringAsFixed(0)}',
                    color: const Color(0xFF2E7D32), icon: Icons.south_west_rounded)),
                  const SizedBox(width: 10),
                  Expanded(child: _SummaryPill(
                    label: "You'll Give", value: '$sym${toGive.toStringAsFixed(0)}',
                    color: _kRed, icon: Icons.north_east_rounded)),
                ]),
              ),
              ...
              ...list.map((c) {
                ...
                final Color balColor = !hasBal ? Colors.grey.shade400
                    : isPositive ? const Color(0xFF2E7D32) : _kRed;
                        ...
                        child: Padding(
                          ...
                          child: Row(children: [
                            Expanded(child: Column(
                                ...
                                children: [
                                  Text(c.name.trim().isEmpty ? 'Party' : c.name.trim(),
                                      style: const TextStyle(fontSize: 14,
                                          fontWeight: FontWeight.w700,
                                          color: Colors.black87),
                                      maxLines: 1, overflow: TextOverflow.ellipsis),
                                  const SizedBox(height: 3),
                                  Text(DateFormat('d MMM yyyy').format(c.updatedAt),
    ...
    switch (kind) {
      case 'purchase':
        ...
        partyName = row['supplier_name']?.toString().trim().isNotEmpty == true
            ? row['supplier_name'] as String : 'Supplier';
      ...
      case 'payment':
        ...
        partyName = row['party_name']?.toString().trim().isNotEmpty == true
            ? row['party_name'] as String : 'Party';
        refNo   = '#${row['id'] ?? ''}';
      ...
      default: // sale
        ...
        partyName = row['customer_name']?.toString().trim().isNotEmpty == true
            ? row['customer_name'] as String : 'Walk-in';
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                const Spacer(),
                // action icons (sale only)
                if (showActions) ...[
                  _ActionBtn(icon: Icons.print_outlined,   tooltip: 'Print',       onTap: onPrint),
                  const SizedBox(width: 6),
                  _ActionBtn(icon: Icons.share_outlined,   tooltip: 'Share PDF',   onTap: onShare),
                  const SizedBox(width: 6),
                  _ActionBtn(icon: Icons.image_outlined,   tooltip: 'Share Image', onTap: onImage),
                ],
              ]),
            ]),
          ),
...
class _VyaparHeader extends StatelessWidget {
  ...
  final VoidCallback onEditName, onSettings, onNewSale,
      onAddParty, onParties, onDayBook, onPurchase;

  const _VyaparHeader({
    required this.shopName, required this.sym,
    required this.today, required this.toReceive, required this.toPay,
    required this.onEditName, required this.onSettings, required this.onNewSale,
    required this.onAddParty, required this.onParties,
    required this.onDayBook, required this.onPurchase,
  });
      ...
      child: SafeArea(bottom: false, child: Column(children: [
          ...
          child: Row(children: [
            ...
            Expanded(
              child: GestureDetector(
                onTap: onEditName,
                child: Row(children: [
                  Flexible(child: Text(shopName,
                      style: const TextStyle(color: Colors.white, fontSize: 15,
                          fontWeight: FontWeight.w700),
                      maxLines: 1, overflow: TextOverflow.ellipsis)),
                  const SizedBox(width: 4),
                  const Icon(Icons.edit_outlined, color: Colors.white54, size: 11),
                ]))),
        ...
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
            _QuickLink(icon: Icons.add_shopping_cart_rounded,
                label: 'Sale', color: _kRed, onTap: onNewSale),
            _QuickLink(icon: Icons.shopping_bag_outlined,
                label: 'Purchase', color: _kBlue, onTap: onPurchase),
            _QuickLink(icon: Icons.person_add_alt_1_rounded,
                label: 'Add Party', color: const Color(0xFF2E7D32), onTap: onAddParty),
            _QuickLink(icon: Icons.groups_outlined,
                label: 'Parties', color: const Color(0xFFE65100), onTap: onParties),
            _QuickLink(icon: Icons.menu_book_outlined,
                label: 'Day Book', color: const Color(0xFF6A1B9A), onTap: onDayBook),
          ]),
        ),
      ])),
    );
    ...
    return Tooltip(
      ...
      child: GestureDetector(
        ...
        child: Container(
          width: 28, height: 28,
          decoration: BoxDecoration(
            color: onTap == null ? Colors.grey.shade50 : const Color(0xFFEFF6FF),
            borderRadius: BorderRadius.circular(6),
            border: Border.all(
              color: onTap == null ? Colors.grey.shade200 : const Color(0xFFBFDBFE))),
...
class _TypeToggle extends StatelessWidget {
  ...
  final Color color;
  final VoidCallback onTap;
  const _TypeToggle({required this.label, required this.selected,
      required this.color, required this.onTap});
...
class _Field extends StatelessWidget {
  ...
  const _Field({required this.ctrl, required this.label, required this.icon,
      this.autofocus = false, this.caps = TextCapitalization.none, this.keyboard});

  @override
  Widget build(BuildContext context) {
    return TextField(
      controller: ctrl, autofocus: autofocus,
      textCapitalization: caps, keyboardType: keyboard,
      decoration: InputDecoration(
