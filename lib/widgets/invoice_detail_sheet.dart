...
Future<List<Map<String, dynamic>>> _extractItems(
  ...
  // 2. Fallback: query invoice_items table directly
...
Future<void> showInvoiceDetailSheet(
  ...
  if (inv == null) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Invoice not found'), backgroundColor: Colors.red),
    );
    return;
  }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    _headerChip('Total',
                        '$sym${total.toStringAsFixed(0)}', Colors.white),
                    const SizedBox(width: 8),
                    _headerChip('Paid', '$sym${paid.toStringAsFixed(0)}',
                ...
                Row(children: [
                      ...
                      onTap: () async {
                        try {
                          ...
                          await for (final page in Printing.raster(
                              bytes, pages: [0], dpi: 300)) {
                            final imgBytes = await page.toPng();
                            if (context.mounted) {
                              await Printing.sharePdf(
                                bytes: imgBytes,
                                filename:
                                    'Invoice_${invoice['invoice_no']}.png',
                              );
                            }
                            break;
                          }
                  ...
                  Expanded(
                    child: _ActionButton(
                      ...
                      onTap: () async {
                        final err =
                            await ThermalInvoicePrinter.printInvoice(
                          settings: settings,
                          invoice: invoice,
                          items: items,
                        );
            ...
            Row(children: [
              ...
              Expanded(
                child: _ActionButton(
                  ...
                  color: Colors.orange.shade700,
                  onTap: () async {
                    // Re-fetch items from DB to guarantee they're present
                    final printItems = items.isNotEmpty
                        ? items
                        : await _extractItems(
